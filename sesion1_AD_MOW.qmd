...
1.  Comprender el papel del proceso de adquisición y almacenamiento en un proyecto de análisis de datos, junto a buenas prácticas que promuevan la transparencia y replicabilidad.
2.  Aprender a formular preguntas y plantear hipótesis que puedan ser abordadas mediante el análisis de datos.
3.  Desarrollar la habilidad de realizar pruebas de hipótesis y comprender la interpretación de sus resultados.
...
El proceso de análisis comienza con la curiosidad y/o necesidad.cLa formulación de preguntas relevantes que se puedan responder mediante la exploración y el examen de los datos disponibles.
...
![La media muestral se distribuye normal, sin importar la distribución de la variable subyacente](img/promedio_TLC.png)
...
Una primera manera de aproximarnos a los parámetros (particularmente a la esperanza) es mediante la construcción de intervalos de confianza.
...
$ P( -1.96 < \frac{\bar{Y} - \mu_y }{ \frac{\sigma}{\sqrt{n}}}  < 1.96  ) = 0.95 $$

$ P( \bar{y}-\frac{1.96\sigma }{\sqrt{n}} < \mu < \bar{y} + \frac{1.96\sigma }{\sqrt{n}}  ) = 0.95 $
...
Un elemento importante a considerar es que esto no significa que con 95% de certeza el parámetro está exactamente en estos valores.
Por ejemplo, al 95% de confianza con 20 intervalos 19 contendrán el parámetro.
